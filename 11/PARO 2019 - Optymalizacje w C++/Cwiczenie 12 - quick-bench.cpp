...
static void test_max_threads(benchmark::State& state) {
	for (auto _ : state) {
		auto result = parallel_sum(test_data.begin(), test_data.end(), std::thread::hardware_concurrency());
		benchmark::DoNotOptimize(result);
	}
}
BENCHMARK(test_max_threads);

static void test_more_than_max_threads(benchmark::State& state) {
	for (auto _ : state) {
		auto result = parallel_sum(test_data.begin(), test_data.end(), 10 * std::thread::hardware_concurrency());
		benchmark::DoNotOptimize(result);
	}
}
BENCHMARK(test_more_than_max_threads);
